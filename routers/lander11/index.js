...
const orderModel11 = require("../../models/orderModel11");
const orderModel11Abd = require("../../models/orderModel11-abd");
...
router.get("/details", async (req, res) => {
  ...
  const order = await orderModel11.findOne({ orderId: txnid });
  if (!order) {
    return res
      .status(404)
...
router.post("/create-order", async (req, res) => {
  ...
  try {
    ...
    const existingOrder = await orderModel11.findOne({ orderId });
    if (existingOrder) {
    ...
    const order = await orderModel11.create(payload);
...
router.post("/success", async (req, res) => {
  try {
    ...
    const checkIfOrderExists = await orderModel11.findOne({
      orderId: txn.txnid,
    });
...
router.post("/create-order-abd", async (req, res) => {
  ...
  try {
    ...
    const order = await orderModel11Abd.create(payload);
...
router.delete("/delete-order-abd", async (req, res) => {
  ...
  try {
    const result = await orderModel11Abd.deleteMany({ email });
...
router.get("/get-orders", async (req, res) => {
  const orders = await orderModel11.find({}).sort({ createdAt: -1 });
...
router.get("/get-orders-abd", async (req, res) => {
  const orders = await orderModel11Abd.find({}).sort({ createdAt: -1 });
  return res.status(200).json({ success: true, data: orders });
});
...
router.get("/get-order/main", async (req, res) => {
  ...
  try {
    const orders = await orderModel11
      .find({})
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit);
    return res.status(200).json({
      ...
      data: {
        orders,
        currentPage: page,
        totalPages: Math.ceil((await orderModel11.countDocuments()) / limit),
      },
    });
  } catch (error) {
    return res.status(500).json({ error: error.message });
...
router.get("/get-order/main-abd", async (req, res) => {
  const { page = 1, limit = 100 } = req.query;
  try {
    const orders = await orderModel11Abd
      .find({})
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit);
    return res.status(200).json({
      ...
      data: {
        orders,
        currentPage: page,
        totalPages: Math.ceil((await orderModel11Abd.countDocuments()) / limit),
      },
    });
  } catch (error) {
    return res.status(500).json({ error: error.message });
...
router.patch("/delivery-status/:id", async (req, res) => {
  try {
    ...
    const order = await orderModel11.findByIdAndUpdate(
      id,
